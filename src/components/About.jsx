    ...
    <section id="about" className="section-padding py-12 lg:py-16 bg-gradient-to-b from-transparent to-[var(--surface-hover)]/20">
      <div className="container-main">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {aboutBlocks.map((block, index) => (
            <Reveal key={block.label} delay={index * 0.08}>
              <div className="group relative h-full rounded-card border border-[var(--border)] bg-[var(--surface)] p-6 transition-all hover:border-accent/50 hover:shadow-xl hover:shadow-accent/10 overflow-hidden">
          ...
          <div className="mt-12 grid grid-cols-3 gap-4 sm:gap-6">
            {stats.map((stat, index) => (
              <Reveal key={stat.label} delay={index * 0.1}>
                <div className="group rounded-card border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-6 text-center transition-all hover:border-accent/50 hover:shadow-lg hover:shadow-accent/10">
        ...
        <div className="mt-16 grid gap-6 lg:grid-cols-[1fr_1fr]">
          <Reveal>
            <div className="rounded-card border border-[var(--border)] bg-[var(--surface)] p-6 shadow-card">
                ...
                {education.map((item, index) => (
                  <div key={item.degree} className="relative pl-6 group">
                    ...
                    {index !== education.length - 1 && (
                      <div className="absolute left-[5px] top-8 bottom-[-24px] w-0.5 bg-[var(--border)]" />
                    )}
                    {/* Timeline dot */}
                    <span className={`absolute left-0 top-1.5 h-3 w-3 rounded-full border-2 transition-all group-hover:scale-125 ${item.current ? 'border-accent bg-accent shadow-[0_0_8px_rgba(var(--accent-rgb),0.5)]' : 'border-[var(--border-hover)] bg-[var(--surface)] group-hover:border-accent group-hover:bg-accent/20'}`} />
                    <div className="group-hover:translate-x-1 transition-transform">
                      ...
                      {item.details && (
                        <p className="mt-2 text-body-sm text-text-secondary bg-[var(--surface-hover)]/50 rounded px-2 py-1 inline-block">{item.details}</p>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-6 space-y-4">
                {achievements.map((item, index) => (
                  <Reveal key={item} delay={index * 0.05}>
                    <div className="group flex items-start gap-3 p-3 rounded-lg transition-all hover:bg-[var(--surface-hover)]/50 hover:translate-x-1">
